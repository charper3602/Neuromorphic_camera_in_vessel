...
def task(h,coord_detect,frame,input,LED_array=tracker(None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None)): # Main task function for running plotting,displaying, and inputs
        ...
        # print(f"{h}: X:{coord_detect[0]} vs {LED_array[h].coordpresent[0]}Y:{coord_detect[1]} vs {LED_array[h].coordpresent[1]}")
         ...
         if(LED_array[h].ind==False): #case for if the LED has not been filled with a coordinate yet 
              ...
              # print(f"{h}vs {LED_array[h].ind}")
               in_range=rangefinder(coord_detect[0]-40,coord_detect[0]+40,coord_detect[1]-40,coord_detect[1]+40,LED_array) #check the range to see if its a valid new coordinate
               if((in_range==False)):
...
def show_preview(events: dv.EventStore): #PCA function 
   ...
   # cv.imshow("Preview", frame)
    ...
    frame, 40, 40, 40, 40, cv.BORDER_CONSTANT, value=[255, 255, 255]) 
