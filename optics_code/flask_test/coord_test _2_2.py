...
init_data = requests.post(url, json = {"coords":"x:0 y:0*"})
if init_data.status_code == 200:
    print(init_data.status_code)
else:
    #print the response code if there was an error with posting
    print(f'Error: {init_data.status_code}, {init_data.text}')
    ...
    for i in contours:
        ...
        if (((cCount > 9) or (cCount == (len(contours) - 1))) and (coordStr != "")):
            ...
            if response.status_code == 201:
                ...
                #response1 = requests.get('http://127.0.0.1:5500/api/coords/')
