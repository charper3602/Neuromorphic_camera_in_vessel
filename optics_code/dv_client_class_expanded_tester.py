...
class tracker(): #object to do the calculations
  ...
  def add3dPlot(self,x,y): #append data to coordinate and timestamp arrays for 3d plotting
      ...
      return self
def find_first_boolean_occurrence(arr, target_value):
  
    for index, value in enumerate(arr):
        if value == target_value:
            return index
    return -1
def rangefinder(LED_array,coordxmin,coordxmax,coordymin,coordymax):
     ...
     for i in range(10):
         if(((x_array[i]<=coordxmax)and(x_array[i]>=coordxmin))and((y_array[i]<=coordymax)and(y_array[i]>=coordymin))):
             return True
     return False
def task(h,LED_array,coord_detect,frame,input):
        ...
        #print(f"{h}vs {LED_array[h].ind}")
         if((LED_array[h].ind==True)and(coord_detect[1]>=LED_array[h].coordpresent[1]*.9)and(coord_detect[0]>=LED_array[h].coordpresent[0]*.9)and(coord_detect[1]<=LED_array[h].coordpresent[1]*1.1)and(coord_detect[0]<=LED_array[h].coordpresent[0]*1.1)):#check if any of the 10 LEDs are in the movement region
               #check if its inside the movement region
              #print("working")
              ...
              if(input=="Y"or input=="y"): #choose plotting
                ...
                LED_array[h].plot(30,18) #plot and display the pixel count and preform transform to get frq
              ...
              if(input=="N"or input=="n"): #choose no plotting
                (LED_array[h]).pixel_counter(frame)#output the pixel count
                (LED_array[h]).rate()#output the text
                (LED_array[h]).frq(30,18) #display the pixel count and preform transform to get frq
              LED_array[h].calcuated_text= LED_array[h].calcuated_text+LED_array[h].text_code
              if((LED_array[h].success1/(LED_array[h].failure1+LED_array[h].success1)>.5)and((LED_array[h].success2/(LED_array[h].failure2+LED_array[h].success2)<.10))):
         ...
         if(LED_array[h].ind==False):
               #print(f"{h}vs {coord_detect[1]} vs {LED_array[h].coordpresent[1]*1.1}")
               #print(f"{h}vs {LED_array[h].ind}")
               in_range=rangefinder(LED_array,coord_detect[0]*.80,coord_detect[0]*1.2,coord_detect[1]*.80,coord_detect[1]*1.2)
               if((in_range==False)):
    ...
    for i, c in enumerate(contours):
        ...
        if __name__ == '__main__':
           with multiprocessing.Pool(processes=10) as pool:
               results = pool.starmap(task, ((h,LED_array,coord_detect,frame,input) for h in range(10)))
               LED_array=results[1]
               LED_array=frame
