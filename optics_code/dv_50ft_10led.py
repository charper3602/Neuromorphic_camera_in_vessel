...
velConv = 0.545 #=pixel proj. in ft(@ 50ft) divided by 33ms
...
class tracker(): #object to do the calculations
  ...
  def rate(self): #handle changes to error rate and calcuate the codes give the freqencies
            #pattern1
            if((np.abs(self.max_freq)>=1.0)and(np.abs(self.max_freq)<=2.0)): #find if the LED is outputing a 1
            ...
            #pattern2
            if((np.abs(self.max_freq)>=2.8)and(np.abs(self.max_freq)<=3.0)):#find if the LED is outputing a 0
            ...
            #pattern3
            if((np.abs(self.max_freq)>=3.1)and(np.abs(self.max_freq)<=3.6)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern4
            if((np.abs(self.max_freq)>=3.7)and(np.abs(self.max_freq)<=4.0)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern5
            if((np.abs(self.max_freq)>=4.5)and(np.abs(self.max_freq)<=5.0)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern9
            if((np.abs(self.max_freq)>=9)and(np.abs(self.max_freq)<=9.7)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern10
            if((np.abs(self.max_freq)>=10)and(np.abs(self.max_freq)<=10.7)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
...
def task(h,coord_detect,frame,input,LED_array=tracker(None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None)): # Main task function for running plotting,displaying, and inputs
         global sample, radius, counter, maxCounter, velConv
        ...
        # print(f"{h}: X:{coord_detect[0]} vs {LED_array[h].coordpresent[0]}Y:{coord_detect[1]} vs {LED_array[h].coordpresent[1]}")
         ...
         if((LED_array[h].ind==True)and(coord_detect[1]>=LED_array[h].coordpresent[1]-15)and(coord_detect[0]>=LED_array[h].coordpresent[0]-15)and(coord_detect[1]<=LED_array[h].coordpresent[1]+15)and(coord_detect[0]<=LED_array[h].coordpresent[0]+15)):#check if the potenial LED is in the movement region of a currently existing LED
               ...
               if((LED_array[h].successCount >= 5)):
                ...
                velX = (LED_array[h].coordpast[1]-LED_array[h].coordpresent[1])*(velConv)
                velY = (LED_array[h].coordpast[0]-LED_array[h].coordpresent[0])*(velConv)
                if((LED_array[h].success1>10)):#(LED_array[h].success1/(LED_array[h].failure1+LED_array[h].success1)>.5)and((LED_array[h].success2/(LED_array[h].failure2+LED_array[h].success2)<.10))):#check if its a valid code
                      ...
                      #print("working")
                      LED_array[h].vel=(velX,velY)
                      cv.putText(frame,f"ID:{9} X:{LED_array[h].coordpresent[1]}, Y:{LED_array[h].coordpresent[0]}",(LED_array[h].coordpresent[0]+20,LED_array[h].coordpresent[1]), font, .5,(0,0,0),1,cv.LINE_AA)
                      #if((np.abs(LED_array[h].vel[0]*33) >= 0.1)and(np.abs(LED_array[h].vel[1]*33) >= 0.1)):
                      ...
                      #print("working")
                      LED_array[h].vel=(velX,velY)
                      cv.putText(frame,f"ID:{10} X:{round((LED_array[h].coordpresent[1]*0.018),2)}, Y:{round((LED_array[h].coordpresent[0]*0.018),2)}",(LED_array[h].coordpresent[0]+20,LED_array[h].coordpresent[1]), font, .5,(0,0,0),1,cv.LINE_AA)
                      #if((np.abs(LED_array[h].vel[0]*33) >= 0.1)and(np.abs(LED_array[h].vel[1]*33) >= 0.1)):
