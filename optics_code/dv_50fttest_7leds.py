...
class tracker(): #object to do the calculations
  ...
  def rate(self): #handle changes to error rate and calcuate the codes give the freqencies
            #pattern1
            if((np.abs(self.max_freq)>=0.80)and(np.abs(self.max_freq)<=1.00)): #find if the LED is outputing a 1
            ...
            #pattern2
            if((np.abs(self.max_freq)>=1.8)and(np.abs(self.max_freq)<=2.0)):#find if the LED is outputing a 0
            ...
            #pattern3
            if((np.abs(self.max_freq)>=2.2)and(np.abs(self.max_freq)<=2.4)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern4
            if((np.abs(self.max_freq)>=2.6)and(np.abs(self.max_freq)<=2.8)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern5
            if((np.abs(self.max_freq)>=3.0)and(np.abs(self.max_freq)<=3.2)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern6
            if((np.abs(self.max_freq)>=3.4)and(np.abs(self.max_freq)<=3.6)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern7
            if((np.abs(self.max_freq)>=3.8)and(np.abs(self.max_freq)<=4.0)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
            ...
            #pattern8
            if((np.abs(self.max_freq)>=4.2)and(np.abs(self.max_freq)<=4.4)):#find if the LED is outputing a 0
                self.text_code=self.text_code+"0"
         ...
         if((LED_array[h].ind==True)and(coord_detect[1]>=LED_array[h].coordpresent[1]-15)and(coord_detect[0]>=LED_array[h].coordpresent[0]-15)and(coord_detect[1]<=LED_array[h].coordpresent[1]+15)and(coord_detect[0]<=LED_array[h].coordpresent[0]+15)):#check if the potenial LED is in the movement region of a currently existing LED
               ...
               if((LED_array[h].successCount >= 5)):
                ...
                if((LED_array[h].success1/(LED_array[h].failure1+LED_array[h].success1)>.5)and((LED_array[h].success2/(LED_array[h].failure2+LED_array[h].success2)<.10))):#check if its a valid code
                      ...
                      cv.putText(frame,f"ID:{h+1} X:{LED_array[h].coordpresent[1]}, Y:{LED_array[h].coordpresent[0]}",(LED_array[h].coordpresent[0]+20,LED_array[h].coordpresent[1]), font, .5,(0,0,0),1,cv.LINE_AA)
                      #if((np.abs(LED_array[h].vel[0]*33) >= 0.1)and(np.abs(LED_array[h].vel[1]*33) >= 0.1)):
                      cv.putText(frame,f"Velocity:{h+1} X:{round(((LED_array[h].vel[0]/0.033)*2.237),3)}, Y:{round(((LED_array[h].vel[1]/0.033)*2.237),3)}",(LED_array[h].coordpresent[0]+10,LED_array[h].coordpresent[1]+100), font, .5,(0,0,0),1,cv.LINE_AA)
